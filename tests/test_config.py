...
from py_superops.exceptions import SuperOpsConfigurationError
...
class TestSuperOpsConfig:
    ...
    def test_init_basic(self):
        ...
        assert config.api_key == "test-key-1234567890123456789012345678901234567890"
    ...
    def test_base_url_validation_invalid(self):
        ...
        with pytest.raises(ValidationError):
            SuperOpsConfig(
                api_key="test-key-1234567890123456789012345678901234567890",  # pragma: allowlist secret, base_url="not-a-url"
            )
    ...
    def test_datacenter_detection_custom(self):
        ...
        assert config.is_us_datacenter() is False
    ...
    def test_get_headers_additional(self):
        """Test headers with additional custom headers."""
        config = SuperOpsConfig(
            api_key="test-key-1234567890123456789012345678901234567890",  # pragma: allowlist secret,
            base_url="https://api.superops.com/v1",
        )
        additional = {"X-Custom": "value", "X-Request-ID": "123"}
    ...
    @patch.dict(
        os.environ, {"SUPEROPS_API_KEY": "env-key-1234567890123456789012345678901234567890"}
    )
    def test_env_var_loading(self):
        """Test loading configuration from environment variables."""
        config = SuperOpsConfig(base_url="https://api.superops.com/v1")
        assert config.api_key == "env-key-1234567890123456789012345678901234567890"

    @patch.dict(
        os.environ,
        {
            "SUPEROPS_API_KEY": "env-key-1234567890123456789012345678901234567890",
    ...
    def test_config_str_representation(self):
        ...
        assert config.api_key not in str_repr
        assert "SuperOpsConfig" in str_repr
    ...
    def test_config_equality(self):
        ...
        config3 = SuperOpsConfig(
            api_key="different-key-1234567890123456789012345678901234567890",
            base_url="https://api.superops.com/v1",
        )
    ...
    def test_config_immutability_protection(self):
        """Test that sensitive config fields are protected."""
        config = SuperOpsConfig(
            api_key="test-key-1234567890123456789012345678901234567890",  # pragma: allowlist secret,
            base_url="https://api.superops.com/v1",
        )

        # Test that we can't easily access the raw API key in logs
